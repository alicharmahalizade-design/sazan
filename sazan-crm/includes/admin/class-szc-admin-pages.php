	...
	public static function page_settings() {
		...
		<div class="wrap szc-wrap">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<h2>پیامک بر اساس نتیجه‌ی تماس (خودکار)</h2>
				<table class="form-table"><tbody>
					<tr><td colspan="2"><p class="description">برای هر نتیجه‌ی تماس، قالب پیامکی مشخص کنید تا هنگام ثبتِ آن تماس، به‌صورت خودکار زمان‌بندی و ارسال شود (مثلاً برای «پاسخ نداد» پیامک «تماس ناموفق»). «— بدون پیامک —» یعنی برای آن نتیجه پیامکی نرود.</p></td></tr>
					<?php $omap = (array) $s['outcome_templates']; foreach ( SZC_Settings::call_outcomes() as $ok => $olbl ) : ?>
						<tr><th><?php echo esc_html( $olbl ); ?></th><td>
							<select name="outcome_templates[<?php echo esc_attr( $ok ); ?>]">
								<option value="0">— بدون پیامک —</option>
								<?php foreach ( $templates as $t ) : ?>
									<option value="<?php echo (int) $t->id; ?>" <?php selected( (int) ( $omap[ $ok ] ?? 0 ), (int) $t->id ); ?>><?php echo esc_html( $t->name ); ?></option>
								<?php endforeach; ?>
							</select>
						</td></tr>
					<?php endforeach; ?>

...
const cardCopy=enrollModal?$('[data-spp-v3-card-copy]',enrollModal):null;
if(cardCopy){cardCopy.addEventListener('click',async()=>{const holder=$('[data-spp-v3-card]',enrollModal),number=holder?holder.dataset.sppV3Card:'';if(!number)return;try{if(navigator.clipboard&&isSecureContext)await navigator.clipboard.writeText(number);else{const field=document.createElement('textarea');field.value=number;field.setAttribute('readonly','');field.style.position='absolute';field.style.left='-9999px';document.body.appendChild(field);field.select();document.execCommand('copy');field.remove()}cardCopy.textContent='کپی شد';cardCopy.classList.add('is-copied')}catch(e){cardCopy.textContent='کپی نشد'}setTimeout(()=>{cardCopy.textContent='کپی';cardCopy.classList.remove('is-copied')},2200)})}

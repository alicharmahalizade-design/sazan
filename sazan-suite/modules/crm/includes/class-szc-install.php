...
class SZC_Install {
	...
	public static function migrate() {
		...
		// Backfill analytical timestamps for pre-1.16 contacts.
		$table = $wpdb->prefix . 'szc_contacts';
		$wpdb->query( "UPDATE $table SET stage_changed_at=COALESCE(updated_at,created_at) WHERE stage_changed_at IS NULL" );
		$wpdb->query( "UPDATE $table SET won_at=COALESCE(updated_at,created_at) WHERE stage='registered' AND won_at IS NULL" );
		$wpdb->query( "UPDATE $table SET expected_value=deal_value WHERE expected_value=0 AND deal_value>0" );
		$wpdb->query( "UPDATE $table SET final_value=deal_value WHERE final_value=0 AND deal_value>0 AND stage='registered'" );
		$history = $wpdb->prefix . 'szc_contact_history';
		$wpdb->query( "INSERT INTO $history (contact_id,actor_id,event_type,old_value,new_value,duration_hours,created_at)
			SELECT c.id,0,'stage','',c.stage,0,COALESCE(c.created_at,NOW()) FROM $table c
			LEFT JOIN $history h ON h.contact_id=c.id AND h.event_type='stage'
			WHERE h.id IS NULL" );
		...
		if ( class_exists( 'SZC_Templates' ) ) {
			...
			SZC_Templates::migrate_default_salutations_to_last_name();
		}
	}
	...
	public static function create_tables() {
		...
		$au = $wpdb->prefix . 'szc_audit_log';
		$ch = $wpdb->prefix . 'szc_contact_history';
		...
		$sql = "CREATE TABLE $c (
			...
			KEY reserved (reserved_until,reserved_by),
			KEY contact_name (last_name,first_name),
			KEY company (company),
		...
		CREATE TABLE $ch (
			...
			duration_hours decimal(12,2) NOT NULL DEFAULT 0,
			created_at datetime DEFAULT NULL,
			PRIMARY KEY  (id),
			KEY contact_id (contact_id),
		...
		CREATE TABLE $ag (
			...
			active tinyint(1) NOT NULL DEFAULT 1,
			daily_answered_target int(11) NOT NULL DEFAULT 40,
			daily_followup_target int(11) NOT NULL DEFAULT 10,
			daily_conversion_target int(11) NOT NULL DEFAULT 2,
			capacity int(11) NOT NULL DEFAULT 100,
			distribution_weight int(11) NOT NULL DEFAULT 1,

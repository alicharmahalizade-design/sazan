...
class SZC_Admin {
	...
	public static function init() {
		...
		$ajax = array(
			...
			'reserve_lead'  => 'reserve_lead',
			'claim_lead'    => 'claim_lead',
			'log_external'  => 'log_external',
	...
	public static function menu() {
		...
		add_submenu_page( 'szc', 'نمای کلی', 'نمای کلی', $cap, 'szc', array( __CLASS__, 'page_dashboard' ) );
		...
		// ابزارهای کم‌استفاده از منوی روزمره حذف شده‌اند، اما از «مرکز تنظیمات» در دسترس‌اند.
		add_submenu_page( null, 'مخاطبین تکراری', 'مخاطبین تکراری', $cap, 'szc-duplicates', array( 'SZC_Admin_Pages', 'page_duplicates' ) );
		add_submenu_page( null, 'ایمپورت شماره‌ها', 'ایمپورت شماره‌ها', $cap, 'szc-import', array( 'SZC_Admin_Pages', 'page_import' ) );
		add_submenu_page( null, 'قالب‌های پیامک', 'قالب‌های پیامک', $cap, 'szc-templates', array( 'SZC_Admin_Pages', 'page_templates' ) );
		add_submenu_page( null, 'دنباله‌های پیامکی', 'دنباله‌های پیامکی', $cap, 'szc-sequences', array( 'SZC_Admin_Pages', 'page_sequences' ) );
		add_submenu_page( null, 'بخش‌بندی‌ها', 'بخش‌بندی‌ها', $cap, 'szc-segments', array( 'SZC_Admin_Pages', 'page_segments' ) );
		add_submenu_page( null, 'لیست سیاه', 'لیست سیاه', $cap, 'szc-blacklist', array( 'SZC_Admin_Pages', 'page_blacklist' ) );
		add_submenu_page( null, 'سطل زباله', 'سطل زباله', $cap, 'szc-trash', array( 'SZC_Admin_Pages', 'page_trash' ) );
		add_submenu_page( null, 'گزارش تغییرات', 'گزارش تغییرات', $cap, 'szc-audit', array( 'SZC_Admin_Pages', 'page_audit' ) );
		add_submenu_page( null, 'گزارش تحویل پیامک', 'گزارش تحویل پیامک', $cap, 'szc-delivery', array( 'SZC_Admin_Pages', 'page_delivery' ) );
	}
	...
	public static function page_dashboard() {
		...
		<div class="wrap szc-wrap">
			...
			<?php if ( ! SZC_SMS::enabled() ) : ?>
				<div class="notice notice-warning"><p>سرویس پیامک هنوز فعال نیست. برای ارسال پیامک، در <a href="<?php echo esc_url( self::url( 'szc-settings' ) . '#sms' ); ?>">مرکز تنظیمات ← پیامک</a> کلید API و خط ارسال را وارد و فعال کنید.</p></div>
			<?php endif; ?>
	...
	public static function page_contacts() {
		...
		if ( $contact_id ) {
			...
			if ( $c && SZC_Auth::can_access_contact( $c ) ) {
	...
	protected static function render_single( $c ) {
		...
		<div class="wrap szc-wrap szc-single" data-contact="<?php echo (int) $c->id; ?>">
			...
			<div class="szc-single-grid">
				<div class="szc-col">
					...
					<div class="szc-card">
						...
						<?php else : ?>
							...
							<p class="szc-muted">ارسال پیامک فقط از قالب‌های پترن‌دار و تأییدشده انجام می‌شود.</p>
				...
				<div class="szc-col">
					<?php echo SZC_Conversation::render( $c, 'admin' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
					<div class="szc-card">
						<h2>ثبت تماس</h2>
						<div class="szc-form-row">
		...
		<div class="wrap szc-wrap">
			...
			<div class="szc-card szc-add-form" style="max-width:640px">
				<div class="szc-form2">
					...
					<label>ارزش احتمالی (تومان)<input type="number" min="0" step="1" data-f="expected_value"></label>
					<label>مبلغ نهایی (تومان)<input type="number" min="0" step="1" data-f="final_value"></label>
					<label>اولویت
	...
	protected static function require_permission( $permission ) {
		if ( ! SZC_Auth::can( $permission ) ) {
			wp_send_json_error( array( 'msg' => 'برای انجام این عملیات مجوز ندارید.' ), 403 );
		}
	}

	protected static function req_contact( $permission = 'view' ) {
		$id = isset( $_POST['contact'] ) ? absint( $_POST['contact'] ) : 0;
		$c  = $id ? SZC_Contacts::get( $id ) : null;
		if ( ! $c ) {
			wp_send_json_error( array( 'msg' => 'مخاطب یافت نشد.' ) );
		}
		if ( ! SZC_Auth::can_access_contact( $c, $permission ) ) {
			wp_send_json_error( array( 'msg' => 'این مخاطب در محدوده دسترسی شما نیست یا مجوز عملیات را ندارید.' ), 403 );
		}
		return $c;
	}

	protected static function req_related_contact( $kind, $permission ) {
		$id  = isset( $_POST['id'] ) ? absint( $_POST['id'] ) : 0;
		$row = $kind === 'note' ? SZC_Activity::get_note( $id ) : SZC_Activity::get_activity( $id );
		$c   = $row ? SZC_Contacts::get( (int) $row->contact_id ) : null;
		if ( ! $row || ! SZC_Auth::can_access_contact( $c, $permission ) ) {
			wp_send_json_error( array( 'msg' => 'رکورد یافت نشد یا مجوز دسترسی ندارید.' ), 403 );
		}
		return array( $row, $c );
	}
	...
	public static function ajax_save_contact() {
		self::guard();
		$c = self::req_contact( 'edit_contact' );
	...
	protected static function posted_fields() {
		$fields = array( 'first_name', 'last_name', 'mobile', 'job', 'company', 'city', 'email', 'source', 'campaign', 'tags', 'priority', 'stage', 'deal_value', 'expected_value', 'final_value' );
		$out    = array();
		foreach ( $fields as $f ) {
			if ( isset( $_POST[ $f ] ) ) {
	...
	public static function ajax_del_note() {
		self::guard();
		self::req_related_contact( 'note', 'notes' );
	...
	public static function ajax_del_activity() {
		self::guard();
		$id  = isset( $_POST['id'] ) ? absint( $_POST['id'] ) : 0;
		$row = SZC_Activity::get_activity( $id );
		self::req_related_contact( 'activity', ( $row && $row->type === 'followup' ) ? 'followups' : 'calls' );
	...
	public static function ajax_log_call() {
		self::guard();
		$c        = self::req_contact( 'calls' );
		$outcome  = isset( $_POST['outcome'] ) ? sanitize_key( $_POST['outcome'] ) : 'answered';
		$note     = isset( $_POST['note'] ) ? wp_unslash( $_POST['note'] ) : '';
		if ( in_array( $outcome, array( 'not_interested', 'wrong' ), true ) && trim( (string) $note ) === '' ) {
			wp_send_json_error( array( 'msg' => 'برای نتیجه «' . SZC_Settings::outcome_label( $outcome ) . '» ثبت توضیح الزامی است.' ) );
		}
		$followup_at = isset( $_POST['followup_at'] ) ? sanitize_text_field( wp_unslash( $_POST['followup_at'] ) ) : '';
		$followup_ts = $followup_at ? szc_ts_from_datetime( $followup_at ) : 0;
		if ( $followup_at !== '' && ! $followup_ts ) {
			wp_send_json_error( array( 'msg' => 'زمان پیگیری نامعتبر است.' ) );
		}
		if ( $followup_ts && ! SZC_Auth::can( 'followups' ) ) {
			wp_send_json_error( array( 'msg' => 'مجوز ثبت پیگیری ندارید.' ), 403 );
		}
		...
		if ( $followup_ts ) {
			SZC_Activity::add_followup( (int) $c->id, wp_date( 'Y-m-d H:i:s', $followup_ts ), wp_unslash( $_POST['followup_note'] ?? '' ) );
		}
		SZC_Workspace::release( (int) $c->id );
		$msg = 'تماس ثبت شد.';
	...
	public static function ajax_dialer_call() {
		...
		$note     = isset( $_POST['note'] ) ? wp_unslash( $_POST['note'] ) : '';
		if ( in_array( $outcome, array( 'not_interested', 'wrong' ), true ) && trim( (string) $note ) === '' ) {
			wp_send_json_error( array( 'msg' => 'برای این نتیجه ثبت توضیح الزامی است.' ) );
		}
		$template = isset( $_POST['template'] ) ? absint( $_POST['template'] ) : 0;
		$want     = true;
	...
	public static function ajax_custom_sms() {
		self::guard();
		$c    = self::req_contact( 'sms' );
		$text = sanitize_textarea_field( wp_unslash( $_POST['text'] ?? '' ) );
		if ( $text === '' ) {
			wp_send_json_error( array( 'msg' => 'متن پیام را وارد کنید.' ) );
		}
		if ( ! SZC_SMS::is_sendable( $c->mobile, $c ) ) {
			wp_send_json_error( array( 'msg' => 'این مخاطب لغو دریافت دارد یا در لیست سیاه است.' ) );
		}
		$res = SZC_SMS::send_text( $c->mobile, SZC_Templates::fill( $text, SZC_Contacts::vars( $c ) ) );
		SZC_Activity::log( (int) $c->id, 'sms', array(
			'outcome' => ! empty( $res['ok'] ) ? 'sent' : 'failed',
			'body'    => $text,
			'meta'    => array( 'source' => 'custom', 'response' => $res['msg'] ?? '' ),
		) );
		if ( empty( $res['ok'] ) ) {
	...
	public static function ajax_done_followup() {
		self::guard();
		self::req_related_contact( 'activity', 'followups' );
	...
	public static function ajax_schedule_sms() {
		...
		if ( ! SZC_SMS::schedule_thanks( $c, $tid, 'manual_schedule' ) ) {
	...
	public static function ajax_del_contact() {
		self::guard();
		$c = self::req_contact( 'delete_contact' );
		SZC_Contacts::delete( (int) $c->id );
		wp_send_json_success( array( 'msg' => 'مخاطب به سطل زباله منتقل شد.', 'redirect' => self::url( 'szc-contacts' ) ) );
	}
	...
	public static function ajax_group_create() {
		self::guard();
		self::require_permission( 'manage_groups' );
	...
	public static function ajax_set_group() {
		self::guard();
		$c   = self::req_contact( 'move_groups' );
	...
	public static function ajax_group_bulk() {
		self::guard();
		self::require_permission( ( ( $_POST['op'] ?? '' ) === 'sms' ) ? 'bulk_sms' : 'edit_contact' );
		$gid = isset( $_POST['group'] ) ? absint( $_POST['group'] ) : 0;
		$op  = isset( $_POST['op'] ) ? sanitize_key( $_POST['op'] ) : '';
		$args = array( 'group' => $gid );
		...
		if ( $op === 'stage' ) {
			...
			wp_send_json_success( array( 'msg' => 'پیامک برای ' . szc_fa_digits( $r['queued'] ) . ' مخاطب در صف قرار گرفت (' . szc_fa_digits( $r['skipped'] ) . ' رد شد).', 'reload' => true ) );
		}
	...
	public static function ajax_list_bulk() {
		self::guard();
		$op    = isset( $_POST['op'] ) ? sanitize_key( $_POST['op'] ) : '';
		$needed = $op === 'sms' ? 'bulk_sms' : ( $op === 'move' ? 'move_groups' : 'edit_contact' );
		self::require_permission( $needed );
		$value = isset( $_POST['value'] ) ? sanitize_text_field( wp_unslash( $_POST['value'] ) ) : '';
		...
		if ( $op === 'sms' ) {
			...
			$message_type = isset( $_POST['message_type'] ) ? sanitize_key( $_POST['message_type'] ) : 'template';
			if ( $message_type === 'free' ) {
				$text = sanitize_textarea_field( wp_unslash( $_POST['text'] ?? '' ) );
				if ( trim( $text ) === '' ) {
					wp_send_json_error( array( 'msg' => 'متن پیام را وارد کنید.' ) );
				}
				if ( ! SZC_SMS::free_text_ready() ) {
					wp_send_json_error( array( 'msg' => 'خط ارسال‌کننده برای متن آزاد تنظیم نشده است.' ) );
				}
				$r = SZC_SMS::enqueue_text_bulk( $ids, $text );
			} else {
				$tid = absint( $value );
				if ( ! $tid ) {
					wp_send_json_error( array( 'msg' => 'پیام آماده را انتخاب کنید.' ) );
				}
				$r = SZC_SMS::enqueue_template_bulk( $ids, $tid );
			}
			if ( ! empty( $r['error'] ) ) {
				wp_send_json_error( array( 'msg' => $r['error'] ) );
			}
	...
	public static function ajax_merge() {
		...
		$other_contact = SZC_Contacts::get( $other );
		if ( ! SZC_Auth::can_access_contact( $other_contact, 'merge' ) ) {
			wp_send_json_error( array( 'msg' => 'مخاطب دوم در محدوده دسترسی شما نیست.' ), 403 );
		}
	...
	public static function ajax_reserve_lead() {
		self::guard();
		$c = self::req_contact( 'calls' );
		$res = SZC_Workspace::reserve_for_call( (int) $c->id );
		empty( $res['ok'] ) ? wp_send_json_error( array( 'msg' => $res['msg'] ), 409 ) : wp_send_json_success( $res );
	}

	public static function ajax_claim_lead() {
		self::guard();
		$id  = isset( $_POST['contact'] ) ? absint( $_POST['contact'] ) : 0;
		$res = SZC_Workspace::claim( $id );
		empty( $res['ok'] ) ? wp_send_json_error( array( 'msg' => $res['msg'] ), 409 ) : wp_send_json_success( array( 'msg' => $res['msg'], 'reload' => true ) );
	}

	public static function ajax_log_external() {
		self::guard();
		$c = self::req_contact( 'calls' );
		$channel = isset( $_POST['channel'] ) && $_POST['channel'] === 'telegram' ? 'telegram' : 'whatsapp';
		SZC_Activity::log( (int) $c->id, 'external', array(
			'outcome' => $channel,
			'body' => ( $channel === 'telegram' ? 'تلگرام' : 'واتساپ' ) . ' از داخل CRM باز شد.',
		) );
		wp_send_json_success( array( 'msg' => 'تعامل در تایم‌لاین ثبت شد.' ) );
	}

	public static function ajax_today_alerts() {
		self::guard();
		$owner = SZC_Settings::scope_owner();
		$due = SZC_Activity::due_followups( 20, $owner );
		$upcoming = SZC_Workspace::upcoming( $owner, 2, 20 );
		$forgotten = SZC_Workspace::unattended( $owner, 20 );
		wp_send_json_success( array(
			'due' => count( $due ),
			'upcoming' => count( $upcoming ),
			'forgotten' => count( $forgotten ),
			'message' => count( $due ) ? szc_fa_digits( count( $due ) ) . ' پیگیری سررسیده دارید.' : ( count( $upcoming ) ? szc_fa_digits( count( $upcoming ) ) . ' پیگیری نزدیک دارید.' : '' ),
		) );
	}
}

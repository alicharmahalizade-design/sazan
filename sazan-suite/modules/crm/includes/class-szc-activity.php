...
class SZC_Activity {
	...
	public static function add_note( $contact_id, $body, $user_id = 0 ) {
		...
		if ( class_exists( 'SZC_Audit' ) ) {
			SZC_Audit::log( 'note_add', 'contact', $contact_id, 'یادداشت اضافه شد', null, array( 'note_id' => $id, 'body' => $body ) );
		}
		return $id;
	}
	...
	public static function delete_note( $id ) {
		global $wpdb;
		$before = self::get_note( $id );
		$wpdb->delete( self::t_notes(), array( 'id' => (int) $id ) );
		if ( $before && class_exists( 'SZC_Audit' ) ) {
			SZC_Audit::log( 'note_delete', 'contact', (int) $before->contact_id, 'یادداشت حذف شد', $before, null );
		}
	}

	public static function get_note( $id ) {
		global $wpdb;
		return $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . self::t_notes() . ' WHERE id=%d', (int) $id ) );
	}

	public static function get_activity( $id ) {
		global $wpdb;
		return $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . self::t_act() . ' WHERE id=%d', (int) $id ) );
	}
	...
	public static function log( $contact_id, $type, $args = array() ) {
		...
		$id = (int) $wpdb->insert_id;
		if ( class_exists( 'SZC_Audit' ) && in_array( $type, array( 'call', 'followup' ), true ) ) {
			SZC_Audit::log( sanitize_key( $type ) . '_add', 'contact', $contact_id, $type === 'call' ? 'تماس ثبت شد' : 'پیگیری ثبت شد', null, array( 'activity_id' => $id, 'outcome' => $a['outcome'] ) );
		}
		return $id;
	}

	/** آیا در چند دقیقه‌ی اخیر برای این مخاطب پیامک فوریِ موفق ثبت شده است؟ */
	public static function has_recent_immediate_sms( $contact_id, $minutes = 15 ) {
		global $wpdb;
		$since = wp_date( 'Y-m-d H:i:s', time() - max( 1, (int) $minutes ) * MINUTE_IN_SECONDS );
		return (bool) $wpdb->get_var( $wpdb->prepare(
			'SELECT id FROM ' . self::t_act()
			. " WHERE contact_id=%d AND type='sms' AND outcome='sent' AND created_at>=%s AND meta LIKE %s LIMIT 1",
			(int) $contact_id,
			$since,
			'%"source":"immediate"%'
		) );
	...
	public static function log_call( $contact, $outcome, $body = '', $want_sms = null, $template_id = 0 ) {
		...
		$scheduled = false;
		if ( $want_sms !== false && ! $contact->opt_out && ! self::has_recent_immediate_sms( $cid ) ) {
	...
	public static function delete_activity( $id ) {
		global $wpdb;
		$before = self::get_activity( $id );
		$wpdb->delete( self::t_act(), array( 'id' => (int) $id ) );
		if ( $before && class_exists( 'SZC_Audit' ) ) {
			SZC_Audit::log( 'activity_delete', 'contact', (int) $before->contact_id, 'فعالیت حذف شد', $before, null );
		}
	}
	...
	public static function latest_followup( $contact_id ) {
		global $wpdb;
		return $wpdb->get_row( $wpdb->prepare(
			'SELECT * FROM ' . self::t_act() . " WHERE contact_id=%d AND type='followup' ORDER BY created_at DESC LIMIT 1",
			(int) $contact_id
		) );
	}
	...
	public static function followups( $when = 'due', $owner = 0, $limit = 200 ) {
		...
		$sql  = 'SELECT a.*, c.first_name, c.last_name, c.mobile, c.owner_id FROM ' . self::t_act() . ' a '
			...
			. "WHERE c.deleted_at IS NULL AND a.type='followup' AND a.done=0 AND a.due_at IS NOT NULL";
		$args = array();
		if ( $when === 'due' ) {
	...
	public static function run_followup_reminders() {
		...
		$rows = $wpdb->get_results( $wpdb->prepare(
			...
			. "WHERE c.deleted_at IS NULL AND a.type='followup' AND a.done=0 AND a.reminded=0 AND a.due_at IS NOT NULL AND a.due_at<=%s "
		...
		foreach ( $rows as $r ) {
			...
			if ( $sms && SZC_SMS::enabled() ) {
				...
				if ( $to !== '' ) {
					SZC_SMS::send_system_pattern( $to, $msg );
				}
			}

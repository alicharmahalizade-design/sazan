...
class SZP_Eval {
	...
	/** ثبت تارگت برای همه جلسه‌های شروع‌شده بدون محدودیت روز باز است. */
	public static function target_open( $n ) {
		$n = (int) $n;
		return $n >= 1 && $n <= self::current_session();
	}

	/** زمان دقیق بازشدن ثبت تارگت جلسه N: ابتدای سه‌شنبه، به وقت سایت. */
	public static function target_open_timestamp( $n ) {
		$dt = self::session_date( $n );
		$dt->setTime( 0, 0, 0 );
		return $dt->getTimestamp();
	}

	/** ثبت نتیجه برای همه جلسه‌های شروع‌شده بدون محدودیت روز باز است. */
	public static function result_open( $n ) {
		$n = (int) $n;
		return $n >= 1 && $n <= self::current_session();
	}

	/** زمان دقیق بازشدن ثبت نتیجه جلسه N: ابتدای دوشنبه هفته بعد، به وقت سایت. */
	public static function result_open_timestamp( $n ) {
		$dt = self::session_date( $n );
		$dt->modify( '+6 days' );
		$dt->setTime( 0, 0, 0 );
		return $dt->getTimestamp();
	...
	public static function edit_target( $user_id, $week_no, $amount ) {
		...
		$before = class_exists( 'SZP_Growth' ) ? SZP_Growth::row_payload( $row ) : array();
		$now = current_time( 'mysql' );
		$wpdb->update( self::table(),
			array( 'target' => $amount, 'target_set_at' => $now, 'updated_at' => $now ),
			array( 'id' => (int) $row->id ), array( '%f', '%s', '%s' ), array( '%d' ) );
		if ( class_exists( 'SZP_Growth' ) ) {
			SZP_Growth::record_revision( $user_id, $week_no, 'edit_target', $before, SZP_Growth::row_payload( self::get_week( $user_id, $week_no ) ) );
		}
		return array(
			'ok'     => true,
			'week'   => $week_no,
	...
	public static function edit_result( $user_id, $week_no, $amount ) {
		...
		if ( class_exists( 'SZP_Growth' ) ) {
			SZP_Growth::record_revision( $user_id, $week_no, 'edit_result', $before, SZP_Growth::row_payload( self::get_week( $user_id, $week_no ) ) );
		}
		return array(
			'ok'     => true,
			'week'   => $week_no,
	...
	public static function delete_week( $user_id, $week_no ) {
		global $wpdb;
		$row = self::get_week( $user_id, $week_no );
		if ( $row && class_exists( 'SZP_Growth' ) ) {
			SZP_Growth::record_revision( $user_id, $week_no, 'delete', SZP_Growth::row_payload( $row ), array() );
		}
	...
	public static function statuses() {
		return array(
			'beyond'  => array( 'label' => szp_ui_text( 'status_beyond', 'بیشتر از هدف' ), 'color' => '#0284c7', 'emoji' => '↑', 'msg' => 'فروش واقعی از هدف بیشتر شده است.' ),
			'success' => array( 'label' => szp_ui_text( 'status_success', 'هدف کامل محقق شد' ), 'color' => '#15803d', 'emoji' => '✓', 'msg' => 'فروش واقعی به هدف رسیده است.' ),
			'improve' => array( 'label' => szp_ui_text( 'status_improve', 'نزدیک به هدف' ), 'color' => '#b45309', 'emoji' => '≈', 'msg' => 'فروش واقعی به هدف نزدیک است.' ),
			'ontrack' => array( 'label' => szp_ui_text( 'status_ontrack', 'کمتر از هدف' ), 'color' => '#b91c1c', 'emoji' => '↓', 'msg' => 'فروش واقعی هنوز کمتر از هدف است.' ),
			'pending' => array( 'label' => szp_ui_text( 'growth_pending_label', 'نتیجه ثبت نشده' ), 'color' => '#475569', 'emoji' => '…', 'msg' => 'فروش واقعی این هفته هنوز وارد نشده است.' ),
		);
	}
	...
	public static function render( $atts = array() ) {
		$title    = ( isset( $atts['title'] ) && $atts['title'] !== '' ) ? $atts['title'] : szp_ui_text( 'growth_title', 'ارزیابی هفتگی من' );
		...
		$has_coaching  = SZP_Growth::has_coaching_access( $uid );
		// The four evaluation roles are intentionally isolated from the student
		// coaching area. Users without an evaluation role keep their course-based
		// coaching access.
		$show_coaching = $has_coaching && ! $role_slug;
		...
		ob_start(); ?>
		<div class="szp">
			<div class="szp-eval2 szp-growth" data-currency="<?php echo esc_attr( $currency ); ?>" data-near="<?php echo esc_attr( self::near() ); ?>">
				<div class="szp-ev2-top">
					<div>
						<span class="szp-growth-kicker">داشبورد ارزیابی</span>
				...
				<?php if ( $role_label ) : ?>
					<div class="szp-growth-role-mobile is-<?php echo esc_attr( sanitize_html_class( $role_slug ?: 'user' ) ); ?>" aria-label="<?php echo esc_attr( 'نقش شما: ' . $role_label ); ?>">
						<span>نقش شما</span>
						<b><?php echo esc_html( $role_label ); ?></b>
					</div>
				<?php endif; ?>

				<?php
				if ( $can_register ) {
					echo SZP_Growth::self_panel( $uid, $currency, $sees_team, $show_coaching ); // phpcs:ignore WordPress.Security.EscapeOutput
				}
				if ( $sees_team ) {
					echo SZP_Growth::team_panel( $uid, $currency ); // phpcs:ignore WordPress.Security.EscapeOutput
				}
				if ( $show_coaching ) {
					echo SZP_Growth::coaching_panel( $uid ); // phpcs:ignore WordPress.Security.EscapeOutput
				}
				?>
			</div>
		</div>
	...
	public static function summary_html( $weeks ) {
		$done = 0; $hit = 0; $sum = 0;
		foreach ( $weeks as $w ) {
			if ( ! $w->has_result ) {
				continue;
	...
	public static function history_html( $weeks, $currency, $editable = false ) {
		if ( ! $weeks ) {
			...
			<div class="szp-ev2-hlist">
				<?php foreach ( $list as $w ) :
					...
					<div class="szp-ev2-hrow">
						...
						<?php endif; ?>
						<?php if ( class_exists( 'SZP_Growth' ) ) { echo SZP_Growth::history_details( $w ); /* phpcs:ignore WordPress.Security.EscapeOutput */ } ?>
					</div>
				<?php endforeach; ?>
			</div>
		</div>

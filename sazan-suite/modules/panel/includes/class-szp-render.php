...
class SZP_Render {
	...
	public static function course_identity( $course_id ) {
		...
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'course_identity_title', 'اطلاعات و شناسنامه دوره' ) ); ?></h3>
			...
			<?php endif; ?>
			<?php if ( $syll ) : ?>
				<h4><?php echo esc_html( szp_ui_text( 'course_outline_title', 'سرفصل‌ها' ) ); ?></h4>
				<ul class="szp-list">
			...
			<?php endif; ?>
			<?php if ( $goals ) : ?>
				<h4><?php echo esc_html( szp_ui_text( 'course_goals_title', 'اهداف نهایی' ) ); ?></h4>
	...
	public static function course_schedule( $course_id ) {
		...
		<section class="szp-sec szp-sec-schedule">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'course_schedule_title', 'زمان‌بندی جلسات' ) ); ?></h3>
			<?php if ( $ordered ) : ?><p class="szp-sched-hint"><?php echo esc_html( szp_ui_text( 'course_schedule_hint', 'برای دیدن هر جلسه، روی آن بزنید.' ) ); ?></p><?php endif; ?>

			<div class="szp-sched-classic">
				<?php if ( $next ) : $n = $next; ?>
					<div class="szp-next">
						<div class="szp-next-info">
							<span class="szp-badge"><?php echo esc_html( szp_ui_text( 'course_next_session', 'جلسه بعدی' ) ); ?></span>
				...
				<?php endif; ?>
				<?php if ( $past ) : ?>
					<h4><?php echo esc_html( szp_ui_text( 'course_past_sessions', 'جلسات برگزارشده' ) ); ?></h4>
				...
				<?php elseif ( ! $next ) : ?>
					<p class="szp-text"><?php echo esc_html( szp_ui_text( 'course_no_sessions', 'هنوز جلسه‌ای ثبت نشده است.' ) ); ?></p>
				<?php endif; ?>
			</div>
			...
			<div class="szp-sched-poster">
				<?php if ( $ordered ) : ?>
					...
					<?php if ( count( $ordered ) > 8 ) : ?>
						<div class="szp-more-wrap"><button type="button" class="szp-more-btn"><?php echo esc_html( szp_ui_text( 'show_more', 'مشاهده بیشتر' ) ); ?></button></div>
					<?php endif; ?>
				<?php else : ?>
					<p class="szp-text"><?php echo esc_html( szp_ui_text( 'course_no_sessions', 'هنوز جلسه‌ای ثبت نشده است.' ) ); ?></p>
				<?php endif; ?>
	...
	public static function course_announcements( $course_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'course_announcements', 'تابلو اعلانات کلاس' ) ); ?></h3>
	...
	public static function course_files( $course_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'course_files', 'فایل‌ها و منابع تکمیلی' ) ); ?></h3>
			<div class="szp-files">
	...
	public static function course_workbench( $course_id, $user_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'course_workbench', 'میز کار تمرین‌ها' ) ); ?></h3>
			<table class="szp-table">
				<thead><tr><th><?php echo esc_html( szp_ui_text( 'course_table_session', 'جلسه' ) ); ?></th><th><?php echo esc_html( szp_ui_text( 'course_table_deadline', 'ددلاین' ) ); ?></th><th><?php echo esc_html( szp_ui_text( 'course_table_status', 'وضعیت' ) ); ?></th></tr></thead>
				<tbody>
					<?php foreach ( $alls as $idx => $s ) :
						...
						<tr>
							...
							<td>
								<?php if ( $sub ) : ?>
									<span class="szp-status szp-status-ok"><?php echo esc_html( szp_ui_text( 'course_status_sent', 'ارسال شد' ) ); ?></span>
								<?php else : ?>
									<span class="szp-status szp-status-pending"><?php echo esc_html( szp_ui_text( 'course_status_pending', 'ارسال نشده' ) ); ?></span>
								<?php endif; ?>
							</td>
						</tr>
					<?php endforeach; ?>
	...
	public static function course_group( $user_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'course_group', 'پنل گروه من' ) ); ?></h3>
			<div class="szp-mates">
	...
	public static function course_survey( $course_id, $user_id ) {
		...
		return self::survey_form( 'course', $course_id, $questions, $ans, szp_ui_text( 'course_survey', 'نظرسنجی‌های دوره' ) );
	}
	...
	public static function session_identity( $session_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'session_identity_title', 'شناسنامه جلسه' ) ); ?></h3>
			<?php if ( trim( $s->post_content ) !== '' ) : ?>
				<div class="szp-rich"><?php echo wp_kses_post( apply_filters( 'the_content', $s->post_content ) ); ?></div>
			<?php endif; ?>
			<?php if ( $skeys ) : ?>
				<h4><?php echo esc_html( szp_ui_text( 'session_goals_title', 'اهداف کلیدی' ) ); ?></h4>
	...
	protected static function audio_player( $url, $title ) {
		ob_start(); ?>
		<div class="szp-audio">
			<button type="button" class="szp-ap-play" aria-label="<?php echo esc_attr( szp_ui_text( 'session_play_label', 'پخش' ) ); ?>"><svg class="szp-ic szp-ic-play" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5.14v13.72c0 .8.87 1.28 1.54.85l10.5-6.86a1 1 0 0 0 0-1.7L9.54 4.29A1 1 0 0 0 8 5.14z"/></svg><svg class="szp-ic szp-ic-pause" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 4.5h3.2c.55 0 1 .45 1 1v13c0 .55-.45 1-1 1H7c-.55 0-1-.45-1-1v-13c0-.55.45-1 1-1zM13.8 4.5H17c.55 0 1 .45 1 1v13c0 .55-.45 1-1 1h-3.2c-.55 0-1-.45-1-1v-13c0-.55.45-1 1-1z"/></svg></button>
			<div class="szp-ap-main">
				<div class="szp-ap-top">
					<span class="szp-ap-name"><?php echo esc_html( $title ); ?></span>
					<span class="szp-ap-tools">
						<button type="button" class="szp-ap-speed" data-speed="1" aria-label="<?php echo esc_attr( szp_ui_text( 'session_speed_label', 'سرعت پخش' ) ); ?>">۱x</button>
						<a class="szp-ap-dl" href="<?php echo esc_url( $url ); ?>" download><?php echo esc_html( szp_ui_text( 'session_download_label', 'دانلود' ) ); ?></a>
					</span>
				</div>
	...
	public static function session_pack( $session_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'session_pack_title', 'فایل‌ها و محتوای جلسه' ) ); ?></h3>
			...
			<div class="szp-files">
				<?php if ( $pdf_url ) : ?>
					<a class="szp-file" href="<?php echo esc_url( $pdf_url ); ?>" target="_blank" rel="noopener"><span class="szp-file-ic" aria-hidden="true">PDF</span><?php echo esc_html( szp_ui_text( 'session_pdf_label', 'اسلایدهای جلسه (PDF)' ) ); ?></a>
				<?php endif; ?>
				<?php foreach ( $sfiles as $fid ) :
					...
					<a class="szp-file" href="<?php echo esc_url( $u ); ?>" target="_blank" rel="noopener"><span class="szp-file-ic" aria-hidden="true">↓</span><?php echo esc_html( get_the_title( $fid ) ); ?></a>
				<?php endforeach; ?>
				...
				for ( $j = 0; $j < $sln; $j++ ) :
					...
					<a class="szp-file" href="<?php echo esc_url( $u ); ?>" target="_blank" rel="noopener"><span class="szp-file-ic" aria-hidden="true">↓</span><?php echo esc_html( $l ); ?></a>
				<?php endfor; ?>
			</div>
			<div class="szp-soonbox"><?php echo esc_html( szp_ui_text( 'session_sales_soon', 'فرم ارزیابی فروش شخصی به‌زودی فعال می‌شود.' ) ); ?></div>
		</section>
		<?php return ob_get_clean();
	}
	...
	public static function session_task( $session_id, $user_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'session_assignment_title', 'تکلیف اختصاصی جلسه' ) ); ?></h3>

			<?php if ( $quiz_ok ) : ?>
				<div class="szp-quiz-box">
					<span class="szp-quiz-title"><?php echo esc_html( $quiz->post_title ); ?></span>
					<a class="szp-btn szp-quiz-btn" href="<?php echo esc_url( get_permalink( $quiz_id ) ); ?>"><?php echo esc_html( szp_ui_text( 'session_quiz_entry', 'ورود به تمرین / آزمون' ) ); ?></a>
				</div>
			<?php endif; ?>
			...
			<?php if ( $task !== '' ) : ?>
				...
				<?php if ( $sub ) : ?>
					<div class="szp-submitted">
						<span class="szp-ok-badge">✓ <?php echo esc_html( szp_ui_text( 'session_answer_saved', 'پاسخ شما ثبت شده است.' ) ); ?></span>
						<span class="szp-sub-date"><?php echo esc_html( szp_ui_text( 'session_last_edit', 'آخرین ویرایش' ) ); ?>: <?php echo esc_html( szp_format_datetime( strtotime( $sub->updated_at ) ) ); ?></span>
						<?php if ( $sub->file_id && ( $fu = wp_get_attachment_url( $sub->file_id ) ) ) : ?>
							<a class="szp-file" href="<?php echo esc_url( $fu ); ?>" target="_blank" rel="noopener"><span class="szp-file-ic" aria-hidden="true">↓</span><?php echo esc_html( szp_ui_text( 'session_sent_file', 'فایل ارسالی شما' ) ); ?></a>
						<?php endif; ?>
					</div>
				<?php endif; ?>
				<form class="szp-task-form" enctype="multipart/form-data">
					<input type="hidden" name="session_id" value="<?php echo (int) $session_id; ?>">
					<textarea name="content" rows="4" class="szp-textarea" placeholder="<?php echo esc_attr( szp_ui_text( 'session_answer_placeholder', 'پاسخ خود را بنویسید...' ) ); ?>"><?php echo $sub ? esc_textarea( $sub->content ) : ''; ?></textarea>
					<label class="szp-file-label"><?php echo esc_html( szp_ui_text( 'session_attachment_label', 'پیوست فایل (اختیاری)' ) ); ?>: <input type="file" name="file"></label>
					<div class="szp-form-row">
						<button type="submit" class="szp-btn"><?php echo esc_html( $sub ? szp_ui_text( 'update_answer', 'ویرایش پاسخ' ) : szp_ui_text( 'submit_answer', 'ارسال پاسخ' ) ); ?></button>
	...
	public static function session_checklist( $session_id, $user_id ) {
		...
		ob_start(); ?>
		<section class="szp-sec">
			<h3 class="szp-sec-h"><?php echo esc_html( szp_ui_text( 'session_checklist_title', 'چک‌لیست جلسه' ) ); ?></h3>
			<ul class="szp-check">
			...
			<p class="szp-hint"><?php echo esc_html( szp_ui_text( 'session_checklist_hint', 'تیک هر مورد بلافاصله ذخیره می‌شود.' ) ); ?></p>
		</section>
		<?php return ob_get_clean();
	}
	...
	public static function session_survey( $session_id, $user_id ) {
		...
		return self::survey_form( 'session', $session_id, $questions, $ans, szp_ui_text( 'session_survey_title', 'نظرسنجی بازخورد سریع' ) );
	}
	...
	protected static function survey_form( $context, $object_id, $questions, $ans, $title ) {
		...
		<section class="szp-sec">
			...
			<?php if ( $ans ) : ?><p class="szp-hint"><?php echo esc_html( szp_ui_text( 'session_survey_answered', 'شما قبلاً پاسخ داده‌اید؛ می‌توانید ویرایش کنید.' ) ); ?></p><?php endif; ?>
			<form class="szp-survey-form">
				...
				<?php endforeach; ?>
				<div class="szp-form-row">
					<button type="submit" class="szp-btn"><?php echo esc_html( szp_ui_text( 'submit_feedback', 'ثبت نظر' ) ); ?></button>
					<span class="szp-form-msg"></span>
				</div>
			</form>
		</section>

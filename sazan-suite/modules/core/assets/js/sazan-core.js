( function( $ ) {
	...
	function initSearch( $h ) {
		...
		$wrap.each( function() {
			...
			$toggle.off( 'click.sz' ).on( 'click.sz', function( e ) {
				e.preventDefault();
				var $portal = $s.data( 'sz-glass-panel' );
				if ( $portal && $portal.length ) {
					$portal.trigger( $portal.hasClass( 'sz-popover-open' ) ? 'sz:close' : 'sz:open' );
					$s.toggleClass( 'is-open', $portal.hasClass( 'sz-popover-open' ) );
				} else {
		...
		$h.find( '.sazan-header__account-wrap, .sazan-header__cart-wrap' ).each( function() {
			...
			if ( $w.data( 'trigger' ) === 'click' ) {
				$w.children( 'a' ).off( 'click.sz' ).on( 'click.sz', function( e ) {
					e.preventDefault();
					var $portal = $w.data( 'sz-glass-panel' );
					if ( $portal && $portal.length ) {
						$portal.trigger( $portal.hasClass( 'sz-popover-open' ) ? 'sz:close' : 'sz:open' );
						$w.toggleClass( 'is-open', $portal.hasClass( 'sz-popover-open' ) );
					} else {
	...
	/*
	 * Popoverهای شیشه‌ای را از هدرِ blurشده خارج می‌کنیم. backdrop-filter
	 * تو‌در‌تو در مرورگرها قابل اتکا نیست؛ اتصال مستقیم به body تضمین می‌کند
	 * حساب، سبد و جستجو هرکدام backdrop مستقل خودشان را داشته باشند.
	 */
	function initGlassPopovers( $h ) {
		if ( ! $h.hasClass( 'skin-glass-one' ) ) { return; }
		var hid = $h.attr( 'data-sz-popover-id' ) || ( 'szp-' + Math.random().toString( 36 ).slice( 2 ) );
		$h.attr( 'data-sz-popover-id', hid );
		var specs = [
			[ '.sazan-header__search', '.sazan-header__search-panel', '.sazan-header__search-toggle' ],
			[ '.sazan-header__account-wrap', '.sazan-header__acc-dropdown', '> a' ],
			[ '.sazan-header__cart-wrap', '.sazan-header__cart-dropdown', '> a' ]
		];

		function closeOthers( panel ) {
			$( '.sazan-glass-popover.sz-popover-open' ).not( panel ).trigger( 'sz:close' );
		}

		specs.forEach( function( spec, index ) {
			$h.find( spec[0] ).each( function() {
				var $wrap = $( this ), $panel = $wrap.find( spec[1] ).first();
				if ( ! $panel.length || $panel.data( 'sz-portaled' ) ) { return; }
				var $trigger = '> a' === spec[2] ? $wrap.children( 'a' ).first() : $wrap.find( spec[2] ).first();
				if ( ! $trigger.length ) { return; }
				$panel.data( 'sz-portaled', true ).addClass( 'sazan-glass-popover' ).attr( 'data-sz-owner', hid );
				$wrap.data( 'sz-glass-panel', $panel );
				$( document.body ).append( $panel );
				var timer = null;

				function place() {
					var r = $trigger[0].getBoundingClientRect();
					var pw = $panel.outerWidth() || 340;
					var left = Math.max( 12, Math.min( r.left, window.innerWidth - pw - 12 ) );
					var top = Math.min( window.innerHeight - 20, r.bottom + 12 );
					$panel.css( { position:'fixed', left:left + 'px', right:'auto', top:top + 'px' } );
				}
				$panel.off( 'sz:open sz:close' )
					.on( 'sz:open', function() { closeOthers( $panel ); place(); $panel.addClass( 'sz-popover-open' ); $wrap.addClass( 'is-open' ); } )
					.on( 'sz:close', function() { $panel.removeClass( 'sz-popover-open' ); $wrap.removeClass( 'is-open' ); } );

				// تریگرهای hover نیز پس از portal شدن بدون اتکا به :hover والد کار می‌کنند.
				if ( $wrap.data( 'trigger' ) !== 'click' && index > 0 ) {
					$wrap.off( '.szportal' ).on( 'mouseenter.szportal', function() { clearTimeout( timer ); $panel.trigger( 'sz:open' ); } )
						.on( 'mouseleave.szportal', function() { timer = setTimeout( function() { $panel.trigger( 'sz:close' ); }, 180 ); } );
					$panel.on( 'mouseenter.szportal', function() { clearTimeout( timer ); } )
						.on( 'mouseleave.szportal', function() { timer = setTimeout( function() { $panel.trigger( 'sz:close' ); }, 180 ); } );
				}
				$( window ).on( 'resize.szportal-' + hid + ' scroll.szportal-' + hid, function() { if ( $panel.hasClass( 'sz-popover-open' ) ) { place(); } } );
			} );
		} );

		$( document ).off( 'click.szportal-' + hid ).on( 'click.szportal-' + hid, function( e ) {
			if ( ! $( e.target ).closest( '.sazan-glass-popover, .sazan-header__search, .sazan-header__account-wrap, .sazan-header__cart-wrap' ).length ) {
				$( '.sazan-glass-popover[data-sz-owner="' + hid + '"]' ).trigger( 'sz:close' );
			}
		} );
	}
	...
	function initSticky( $h ) {
		...
		function stick( el ) {
			...
			var glassInner = el.classList.contains( 'sazan-glass-one-row' ) ? el.querySelector( '.sazan-header__bottom-inner' ) : null;
			var glassInnerStyle = null;
			if ( glassInner ) {
				var gis = getComputedStyle( glassInner );
				glassInnerStyle = {
					maxWidth: gis.maxWidth, paddingTop: gis.paddingTop, paddingRight: gis.paddingRight,
					paddingBottom: gis.paddingBottom, paddingLeft: gis.paddingLeft,
					minHeight: gis.minHeight, gap: gis.gap
				};
			}
			info = { el: el, parent: el.parentNode, next: el.nextSibling, glassInner: glassInner };
			...
			if ( floated ) {
				...
				// هدر تک‌ردیفه باید در حالت عادی و چسبان دقیقاً یک عرض و فاصله
				// بیرونی داشته باشد؛ پس قاب اصلی هدر مبنای اندازه‌گیری است.
				var r0 = el.classList.contains( 'sazan-glass-one-row' )
					? header.getBoundingClientRect()
					: ( innerEl ? innerEl.getBoundingClientRect() : el.getBoundingClientRect() );
			...
			if ( floated ) {
				...
				if ( glassInner && glassInnerStyle ) {
					glassInner.style.maxWidth = glassInnerStyle.maxWidth;
					glassInner.style.marginLeft = 'auto'; glassInner.style.marginRight = 'auto';
					glassInner.style.paddingTop = glassInnerStyle.paddingTop;
					glassInner.style.paddingRight = glassInnerStyle.paddingRight;
					glassInner.style.paddingBottom = glassInnerStyle.paddingBottom;
					glassInner.style.paddingLeft = glassInnerStyle.paddingLeft;
					glassInner.style.minHeight = glassInnerStyle.minHeight;
					glassInner.style.gap = glassInnerStyle.gap;
				} else {
		...
		function unstick() {
			...
			copyInnerWidths( el, true );
			if ( info.glassInner ) {
				[ 'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft', 'minHeight', 'gap' ].forEach( function ( p ) { info.glassInner.style[ p ] = ''; } );
			}
		...
		function onScroll() {
			...
			// در حالت «فقط ردیف دوم»، عنصر چسبان عمداً از داخل هدر به body
			// منتقل می‌شود؛ بنابراین نباید با مقایسهٔ pickEl() آن را در هر
			// اسکرول دوباره آزاد و چسبان کنیم (علت نوسان محو/ظاهرشدن هدر).
			if ( ! stuck ) {
				var el = pickEl();

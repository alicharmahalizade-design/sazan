...
class Header extends Widget_Base {
	...
	private function content_general() {
		...
		$this->add_responsive_control( 'glass_one_padding', array(
			'label' => esc_html__( 'فاصله داخلی هدر', 'sazan-core' ),
			'type' => Controls_Manager::DIMENSIONS,
			'size_units' => array( 'px', 'em', '%' ),
			'default' => array( 'top' => 0, 'right' => 28, 'bottom' => 0, 'left' => 28, 'unit' => 'px', 'isLinked' => false ),
			'tablet_default' => array( 'top' => 0, 'right' => 20, 'bottom' => 0, 'left' => 20, 'unit' => 'px', 'isLinked' => false ),
			'mobile_default' => array( 'top' => 0, 'right' => 14, 'bottom' => 0, 'left' => 14, 'unit' => 'px', 'isLinked' => false ),
			'selectors' => array( '{{WRAPPER}} .sazan-header.skin-glass-one .sazan-header__bottom-inner' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};' ),
		) );
		...
		$this->add_responsive_control( 'glass_side', array(
			'label' => esc_html__( 'فاصله بیرونی چپ و راست (عادی و چسبان)', 'sazan-core' ), 'type' => Controls_Manager::SLIDER,
			'range' => array( 'px' => array( 'min' => 0, 'max' => 120 ) ), 'default' => array( 'unit' => 'px', 'size' => 12 ),
			'selectors' => array( '{{WRAPPER}} .sazan-header' => '--gs: {{SIZE}}px;' ),
			'description' => esc_html__( 'این فاصله هنگام چسبیدن نیز دقیقاً حفظ می‌شود.', 'sazan-core' ),
		) );
	...
	private function render_header() {
		...
		$render_logo = function() use ( $s, $tag_open, $tag_close ) {
			echo $tag_open;
		...
		/* ---- بالا (در طرح تک‌ردیفه عمداً حذف می‌شود) ---- */
		if ( ! $is_glass_one ) {
		...
		/* ---- پایین ---- */
		echo '<div class="sazan-header__bottom' . ( $is_glass_one ? ' sazan-glass-one-row' : '' ) . '"><div class="sazan-header__bottom-inner">';
		if ( $is_glass_one ) {
			$render_logo();
			echo '<button type="button" class="sazan-header__burger" aria-label="menu"><span></span><span></span><span></span></button>';
		}
		if ( ! $is_glass_one && 'yes' === $s['cats_show'] ) {
		...
		if ( ! $is_glass_one && 'yes' === $s['contact_show'] ) {
			$tel = preg_replace( '/[^0-9+]/', '', (string) $s['contact_phone_raw'] );

...
final class Quiz_SMS {
	...
	private function __construct() {
		...
		add_action( 'sazan_quiz_send_otp', array( $this, 'send_otp' ), 10, 4 );
		if ( is_admin() ) {
	...
	/** ارسال کد ورود آزمون؛ در حالت پترن، کد با متغیر {code} فرستاده می‌شود. */
	public function send_otp( $mobile, $code, $quiz_id = 0, $data = array() ) {
		$s = Quiz_Engine::settings();
		if ( empty( $s['sms_apikey'] ) || empty( $s['sms_sender'] ) ) { return false; }
		if ( 'pattern' === $s['sms_mode'] && ! empty( $s['sms_otp_pattern'] ) ) {
			$body = array( 'code' => $s['sms_otp_pattern'], 'sender' => $s['sms_sender'], 'recipient' => $mobile, 'variable' => array( 'code' => (string) $code ) );
			$url = self::ENDPOINT_PATTERN;
		} else {
			$body = array( 'recipient' => array( $mobile ), 'sender' => $s['sms_sender'], 'message' => 'کد تأیید آزمون سازان: ' . $code );
			$url = self::ENDPOINT_SIMPLE;
		}
		return wp_remote_post( $url, array( 'timeout' => 20, 'headers' => array( 'Content-Type' => 'application/json', 'apikey' => $s['sms_apikey'] ), 'body' => wp_json_encode( $body ) ) );
	}
	...
	public function sanitize( $in ) {
		return array(
			...
			'sms_otp_pattern' => sanitize_text_field( $in['sms_otp_pattern'] ?? '' ),
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table" role="presentation">
					...
					<tr>
						<th><label><?php esc_html_e( 'کد پترن تأیید موبایل', 'sazan-core' ); ?></label></th>
						<td><input type="text" name="sazan_quiz_settings[sms_otp_pattern]" value="<?php echo esc_attr( $s['sms_otp_pattern'] ?? '' ); ?>" class="regular-text" style="direction:ltr">
						<p class="description"><?php esc_html_e( 'برای ارسال OTP در پنل پیامک یک پترن بسازید که متغیر code داشته باشد. در حالت متن آزاد نیازی به این فیلد نیست.', 'sazan-core' ); ?></p></td>
					</tr>
					<tr>

...
class SZP_Coach_Render {
	...
	protected static function view_scan( $j ) {
		...
		<section class="szp-sec szp-co-scan" data-course="<?php echo (int) $j->course_id; ?>">
			...
			<div class="szp-co-grid2">
				<label class="szp-co-fld"><span>نام کسب‌وکار *</span><input type="text" data-f="biz_name" value="<?php echo esc_attr( $d['biz_name'] ?? '' ); ?>"></label>
				<label class="szp-co-fld"><span>حوزه / صنعت</span><input type="text" data-f="industry" value="<?php echo esc_attr( $d['industry'] ?? '' ); ?>" placeholder="مثلاً: فروشگاه آنلاین پوشاک"></label>
	...
	protected static function scan_summary( $j ) {
		...
		foreach ( array( 'biz_name' => 'کسب‌وکار', 'industry' => 'صنعت', 'biz_stage' => 'مرحله', 'employees' => 'کارکنان' ) as $k => $lbl ) {
			if ( ! empty( $d[ $k ] ) ) {

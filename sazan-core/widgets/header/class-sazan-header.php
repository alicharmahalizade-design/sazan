...
class Header extends Widget_Base {
	...
	private function content_general() {
		...
		$this->add_control( 'sticky', array(
			...
			'default' => 'none', 'prefix_class' => 'sazan-sticky-', 'separator' => 'before',
			'options' => array(
		...
		$this->add_responsive_control( 'container_width', array(
			...
			'size_units' => array( 'px', '%' ), 'range' => array( 'px' => array( 'min' => 600, 'max' => 2400 ) ),
			'default' => array( 'unit' => 'px', 'size' => 1760 ),
	...
	private function render_header() {
		...
		printf(
			'<div class="sazan-header skin-%1$s" data-sticky="%2$s" data-sticky-offset="%3$d" data-sticky-smart="%4$s" data-sticky-shrink="%5$s" data-sticky-bg="%6$s">',
			esc_attr( $skin ), esc_attr( $st_mode ), $st_off, esc_attr( $st_smart ), esc_attr( $st_shrink ), esc_attr( $st_bg )
		);
